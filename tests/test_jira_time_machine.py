...
def test_history_has_issue_initial_state_and_changes(jira_time_machine):
    ...
    assert len(proj_0001_records[proj_0001_records[("Record", "type")] == "initial"]) == 1
    assert len(proj_0001_records[proj_0001_records[("Record", "type")] == "change"]) == 2

    # PROJ-0002 has 1 record: 1 initial and no changes
    assert len(proj_0002_records) == 1
    assert len(proj_0002_records[proj_0002_records[("Record", "type")] == "initial"]) == 1
    assert len(proj_0002_records[proj_0002_records[("Record", "type")] == "change"]) == 0
...
def test_history_has_correct_initial_states(jira_time_machine):
    ...
    proj_0001_initial_record = history_df[(history_df[('Record', 'issue_id')] == 'PROJ-0001') & (history_df[('Record', 'type')] == 'initial')].iloc[0]
    assert proj_0001_initial_record[('Tracked', 'Status')] == "New"
    assert proj_0001_initial_record[('Tracked', 'Priority')] == "Minor"

    proj_0002_initial_record = history_df[(history_df[('Record', 'issue_id')] == 'PROJ-0002') & (history_df[('Record', 'type')] == 'initial')].iloc[0]
...
def test_history_has_correct_current_states(jira_time_machine):
    ...
    proj_0001_last_record = history_df[history_df[('Record', 'issue_id')] == 'PROJ-0001'].iloc[-1]
    assert proj_0001_last_record[('Tracked', 'Status')] == "Submitted"
    assert proj_0001_last_record[('Tracked', 'Priority')] == "Major"

    proj_0002_last_record = history_df[history_df[('Record', 'issue_id')] == 'PROJ-0002'].iloc[-1]
